...
export default function AddExpenseModal({show, onclose}){
    ...
    const addCategoryHandler = async () => {
        ...
        try {
            if(title == ''){
                return toast.error("Please, put a title!");
            }
    ...
    return (
        <Modal show={show} onClose={onclose}>
            <div className="flex flex-col gap-4">
                ...
                <input type="number" min={0.01} step={0.01} placeholder="Enter an expense amount" value={expenseAmount} onChange={(e)=> { setExpenseAmount(e.target.value) }} />
            </div>
